...
const Blog = () => {
  ...
  const fetchBlogData = async ()=>{
    ...
    const allBlogs = [...storedBlogs,...blog_data]
    const found = allBlogs.find(item => item._id ==id)
    setData(found)

  }
  const fetchComments = async () =>{
   const storedComments = JSON.parse(localStorage.getItem('comments')) || [];
   const relatedComments = storedComments.filter(c => c.blogId === id);
  ...
  const addComment =async (e) =>{
    ...
    setName('');
    setContent('');
    alert('comment Submiited! Waitng for admin approval.');
  };
